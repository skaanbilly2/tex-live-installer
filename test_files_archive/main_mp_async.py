...
async def downloader(MAX_CPU=2):
    try:
        ...
        args = [arg + (queue, mem_fs,) for arg in args]


        download_tasks = [asyncio.create_task(download(*arg)) for arg in args]
        N_writers = 4
        extract_workers = [asyncio.create_task(extract_write_worker(queue, mem_fs,)) for _ in range(N_writers)]

        while queue.empty():  await asyncio.sleep(0.1)
        await queue.join()
